...
workflow TWISTCGP {
    ...
    // FASTP.out.reads emits a bare Path (not a 1-element list) for single-end, which .join() would mangle.
    FGUMI_EXTRACT(FASTP.out.reads.map { meta, reads -> [meta, reads instanceof List ? reads : [reads], meta.id] })
