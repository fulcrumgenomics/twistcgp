...
- ALIGNBAM now runs under Singularity: ZipperBams writes to an intermediate `.zipped.bam` file that `samtools sort` reads (the literal `/dev/stdout` path fails under Singularity), and `findutils` is added to the container so `find` is present in the conda-only Singularity image (which has no base OS).
