...
For non-human panels or custom references, you may optionally provide a pre-computed scan file using the `--msisensor_scan` parameter. The pipeline will not auto-generate a scan file for msisensor2.
...
If not provided via `--msisensor_scan`, the pipeline will auto-generate a scan file from the reference genome when `--use_msisensor_pro_licensed` is enabled.
